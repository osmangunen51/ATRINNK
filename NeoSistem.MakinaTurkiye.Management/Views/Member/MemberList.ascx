    ...
    <td>
        <%if (item.PhoneItems != null)
            ...
            <%}
                ...
                {  
             if(item.MemberType==(byte)MemberType.FastIndividual)
                {
                    if (item.FastMemberShipType == (byte)FastMembershipType.Phone) { 
                    %>
                     <span style="color:#ad0808"><%:phone.PhoneCulture+" "+phone.PhoneAreaCode+" "+phone.PhoneNumber %></span> <br />
                        <span><a onclick="PhoneActiveMailSend(<%:item.MainPartyId %>)" style="cursor:pointer;" >ŞO.MG</a></span>            
                <%}
                    else
                    {%>
                         <span style="color:#cb9707;"><%:phone.PhoneCulture+" "+phone.PhoneAreaCode+"    "+phone.PhoneNumber %></span> <br />
                   <% }
                }
                ...
                else
                {
                 ...
                 <span style="color:#04890f"><%:phone.PhoneCulture+" "+phone.PhoneAreaCode+" "+phone.PhoneNumber %></span>   
                    ...
                    %>
                    <span style="color:#ad0808;" ><%:phone.PhoneCulture+" "+phone.PhoneAreaCode+" "+phone.PhoneNumber %></span>      
                  <% }
                }
                }
            }
 ...
 %>  
  ...
  <td class="Cell">
    ...
    <div style="float: left;">
      Aktif</div>
    <div style="float: left; background-image: url('/Content/Images/Goodshield.png');
    ...
    <div style="float: left;">
      Pasif</div>
    <div style="float: left; background-image: url('/Content/Images/Errorshield.png');
      height: 16px;">
      <img src="/Content/Images/Errorshield.png" />
    </div>
    <div  style="float:left; margin-left:20px;width:40px; text-align:center;cursor:pointer;">
    <a  onclick="Activation(<%: item.MainPartyId %>);"><img src="/Content/Images/activationicon.png" /></a>
    </div>
    ...
    <td class="Cell">
        <%if (item.FastMemberShipType == (byte)FastMembershipType.Facebook)
          ...
          else if(item.FastMemberShipType==(byte)FastMembershipType.ProductComplain)
          {
              Response.Write("Ş");
          }
          else {
  ...
  <td class="CellEnd" style=" text-align: center; width:140px;">
    ...
    </a><a href="/Member/BrowseDesc1/<%: item.MainPartyId %>">
      <img src="/Content/images/product.png" " />
    </a>
    <div style="float:right; width:30px; text-align:right; cursor:pointer;">
    <a href="/Member/memberactivation/<%=item.MainPartyId %>" id="lightbox_click" rel="superbox[iframe]"> <img src="/Content/Images/ikon_ozel_mesaj_gonder.png" /></a>
    </div>
  </td>
...
<% if (Model.TotalRecord <= 0)
   { %>
<tr class="Row">
  <td colspan="10" class="CellBegin Cell" style="color: #FF0000; font-weight: 700; font-size: 14px;">
    Üye bulunamadı.
  </td>
</tr>
<% } %>
<tr>
  <td class="ui-state ui-state-default" colspan="10" align="right" style="border-color: #DDD;
    border-top: none;">
...
<tr>
  <td class="ui-state ui-state-hover" colspan="10" align="right" style="border-color: #DDD;
    border-top: none;">
    Toplam Kayıt : &nbsp;&nbsp;<strong>
