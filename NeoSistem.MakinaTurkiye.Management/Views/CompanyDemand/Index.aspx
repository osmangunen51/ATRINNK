<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<ICollection<NeoSistem.MakinaTurkiye.Management.Models.Entities.CompanyDemandMembership>>" %>
...
<% foreach (var item in Model)
   { %>
<% row++; %>
<tr id="row<%: item.CompanyDemandMembershipId %>" class="<%: (row % 2 == 0 ? "Row" : "RowAlternate") %>">
  <td class="CellBegin">
    <%: item.NameSurname %>
  </td>
    <td>
    ...
    <td style="color:#d30202">
         Aranmadı<br />     
         <a  href="/CompanyDemand?page=<%:page %>&status=1&id=<%:item.CompanyDemandMembershipId %>">Arandı İşaretle</a>
               </td>
         <%} else{%><td style="color:#04d060;">
         Arandı<br />
         <a   href="/CompanyDemand?page=<%:page %>&status=0&id=<%:item.CompanyDemandMembershipId %>">Aranmadı İşaretle</a>
         </td>
             <%} %>
    <td>
    <%  if(item.WebUrl!=null)
  ...
  <td >
    <a style="cursor: pointer;" onclick="DeleteDemand(<%:item.CompanyDemandMembershipId %>)">
