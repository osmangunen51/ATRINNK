<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<ICollection<NeoSistem.MakinaTurkiye.Management.Models.FooterParentModel>>" %>
...
<link rel="stylesheet" href="/Scripts/SuperBox/jquery.superbox.css" type="text/css" media="all" />
  <script type="text/javascript" src="/Scripts/SuperBox/jquery.superbox.js"></script>
    ...
    <script type="text/javascript"> 
        function DeleteFooterParent(ID)
        {
            if (confirm('Kaydı Silmek istediğinizden eminmisiniz ?')) {
                $.ajax({
                    url: '/Footer/FooterParentDelete',
                    data: { id: ID },
                    type: 'post',
                    dataType: 'json',
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <div style="width: 100%; margin: 0 auto;">
      <button style="margin-top:10px;" onclick="window.location='/Footer/FooterParentAdd'" >Yeni Ekle</button>
...
<% foreach (var item in Model)
   { %>
<% row++; %>
<tr id="row<%: item.FooterParentId %>" class="<%: (row % 2 == 0 ? "Row" : "RowAlternate") %>">
  <td class="CellBegin">
    <%: item.FooterParentName %>
  </td>
    <td>
        <a href="/Footer/FooterContents/<%:item.FooterParentId %>">Alt Başlıkları Gör</a>
    </td>
    <td>
        <%:item.DisplayOrder %>
    </td>
  <td >
      <div style="float: left;">
            <a style="cursor: pointer;" onclick="DeleteFooterParent(<%:item.FooterParentId %>)">
        <img src="/Content/images/delete.png" />
    </a>
     <a href="/Footer/FooterParentUpdate/<%: item.FooterParentId %>">
      ...
      <ul>
          <%int pageNumbers = Convert.ToInt32(ViewData["pageNumbers"]);
            for (int i = 1; i <=pageNumbers ; i++)
            {
                ...
                {
                    Response.Write("<li><a href='/Footer/FooterParents?page="+i+"'>" + i + "</a></li>");
                }
