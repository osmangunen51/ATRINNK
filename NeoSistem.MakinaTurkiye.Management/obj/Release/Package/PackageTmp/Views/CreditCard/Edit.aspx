  ...
  <script type="text/javascript">
    ...
    function deleteInstallment(value) {
      if (confirm("Taksit işlemini silmek istediğinizden eminmisiniz ?")) {
        $.ajax({
          ...
          data: { value: value, vpId: $('#vpId').val() },
          cache: false,
    ...
    function SetVirtualPostInstallment(value, count) {
      $.ajax({
        ...
        data: { value: value, vpId: $('#vpId').val(), count: count },
        cache: false,
        success: function (data) { $('#divVirtualPostInstallment').html(data) },
        error: function (x) { alert(x.responseText) }
      });
    ...
    function setVPIKeyupData(targetControl, valOne, valTwo, count) {
      $('#' + targetControl).val(valOne + '&' + valTwo + '&' + count);
    }

  </script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <%using (Html.BeginForm("Edit", "CreditCard", FormMethod.Post, new { enctype = "multipart/form-data" }))
    {%>
  <input type="hidden" id="vpId" value="<%:Model.CreditCard.CreditCardId %>" />
      ...
      <table class="tableForm" style="padding-top: 10px; height: auto;">
        <tr>
          ...
          <td>
              ...
              <%bool active = Convert.ToBoolean(Model.CreditCard.Active); %>
          <td><%:Html.CheckBox("CreditCard.Active",active) %></td>
              </tr>
        <tr>
          <td style="width: 120px;">
            Sanal Pos :
    ...
    <div style="float: left;">
      <div style="float: left; margin-top: 13px; float: left; width: 500px; margin-left: 20px;
        ...
        <div style="float: left; width: 349px;" id="divVirtualPostInstallment">
          <%=Html.RenderHtmlPartial("CreditCardInstallmentForEdit", Model.CreditCardInstallmentItems)%>
        </div>
      </div>
    </div>
  </div>
