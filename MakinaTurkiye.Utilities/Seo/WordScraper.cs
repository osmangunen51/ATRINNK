...
namespace MakinaTurkiye.Utilities.Seo
{
    public static class WordScraper
	{
					...
					foreach (string s in sents)
					{
						if (s.Trim(trim).Length > 2)
						{
							...
							foreach (Match m in mc)
							{
								...
								if (word.Length > 1 || WordIsUncommon(word))	 //all two and one letter words are ignored
								{
									string stem = (word.Length > 1) ? stemmer.Porter.stemTerm(word) : word; //only stem if more than 2 characters
        ...
        private static Regex non20 = new Regex(@"\b(ve|va|veya|vaya|ile|ila)\b", (RegexOptions.IgnoreCase | RegexOptions.Compiled));
        //private static Regex non21 = new Regex(@"\b(bir|iki|sat|daha|gibi|end|bu|yap|olan| ve | veya|ancak|de|bu|tic|ve tic|kdv\dahil|hariç)\b", (RegexOptions.IgnoreCase | RegexOptions.Compiled));

        private static Regex white = new Regex(@"\s+", RegexOptions.Compiled);

		public static Regex WordReg = new Regex(@"(?<=(\s|^))[A-Z0-9\\-ıIçÇüÜğĞöÖşŞ\\.\-]+", (RegexOptions.Compiled | RegexOptions.IgnoreCase));
