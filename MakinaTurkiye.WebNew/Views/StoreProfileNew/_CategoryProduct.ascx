...
<ul class="sub-menu" style="<%: display%>">
    <% if (Model.MTCategoryItems.Count > 0)
        { %>
    <%--Üst Kategoriler--%>
    <%int LeftMargin = 0;  %>
    ...
    <% foreach (var item in Model.MTCategoryItems.Where(c => c.CategoryParentId == Model.ActiveCategory.CategoryId && c.CategoryType != (byte)CategoryType.Model).OrderBy(c => c.CategoryName))
        { %>
