    ...
    public class StoreProfileNewController : BaseController
    {
        ...
        public void PrepareCompanyProfileStoreMemberAndMap(MTCompanyProfileModel model, Store store, int memberMainPartyId)
        {
            ...
            if (!string.IsNullOrEmpty(store.StoreWeb))
            {
                ...
                {
                    if (store.StoreWeb.Contains("trinnk"))
                    {
        ...
        public void PrepareCompanyProfileImageAndVideos(MTCompanyProfileModel model, Store store, int memberMainPartyId)
        {
            ...
            for (int i = 0; i < (int)totalPage; i++)
            {
                ...
                foreach (var item in videos1)
                {
                    ...
                    {
                        VideoImagePath = "//s.trinnk.com/VideoThumb/" + item.VideoPicturePath,
                        VideoPath = "//s.trinnk.com/NewVideos/" + item.VideoPath,
        ...
        public void PrepareCompanyProfileAbout(MTCompanyProfileModel model, Store store)
        {
            ...
            if (store.StorePicture != null)
                ...
                model.MTStoreAboutModel.AboutImagePath = NeoSistem.Trinnk.Web.Helpers.ImageHelpers.GetStoreImage(store.MainPartyId, store.StoreLogo, "300");
        ...
        public void PrepareProductsList(MTStoreProfileProductsModel model, Store store, int PageDimension, int page)
        {
            ...
            foreach (var item in products)
            {
                ...
                mtProductList.ProductDescription = NeoSistem.Trinnk.Core.Web.Helpers.Helpers.Truncate(productDesc, 320);
            ...
            try
            {
                if (!string.IsNullOrEmpty(username))
                {
                    ...
                    if (store != null)
                    {
                        ...
                        {
                            return RedirectPermanent("http://www.trinnk.com/" + request.Url.AbsolutePath.ToLower());
                        }
        ...
        public async Task<ActionResult> CatologNew(string username)
        {
            if (!string.IsNullOrEmpty(username))
            {
                ...
                if (store != null)
                {
                    ...
                    //CreateSeoForStore(store);
                    var request = HttpContext.Request;
                    ViewBag.Canonical = "https://www.trinnk.com" + request.Url.AbsolutePath;
        ...
        public ActionResult AddFavoriteStore(int MainPartyId)
        {
            ...
            ProductUrl = string.Format("/{0}/{1}/{2}/{3}", NeoSistem.Trinnk.Core.Web.Helpers.Helpers.ToUrl("Sirket"), party.MainPartyId, NeoSistem.Trinnk.Core.Web.Helpers.Helpers.ToUrl(party.StoreName), NeoSistem.Trinnk.Core.Web.Helpers.Helpers.ToUrl("SirketProfili"));
