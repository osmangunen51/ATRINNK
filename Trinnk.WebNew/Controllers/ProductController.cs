...
using Trinnk.Utilities.VideoHelpers;
...
using static NeoSistem.Trinnk.Web.Models.Products.MTProductTabModel;
...
namespace NeoSistem.Trinnk.Web.Controllers
{
    ...
    public class ProductController : BaseController
    {
        ...
        private void PrepareProductContactModel(MTProductDetailViewModel productModel, Product product, Store store)
        {
            ...
            string wpMessagecontent = "Merhaba #urunlinki# ilanınız için trinnk.com üzerinden ulaşıyorum";
            ...
            DetailClear(int? productId, string productGroupName, string firstCategoryName, string productName, string pageType)
        {
...
#if !DEBUG
                cookieVisitor.Domain = ".trinnk.com";
#endif
            ...
            for (int Step = 0; Step < productTechnicalInfos.Count(); Step++)
            {
                var item = productTechnicalInfos[Step];
            ...
            var productCommentslist = productComments.Skip(currentPageComment * pageDimension - pageDimension).Take(pageDimension).ToList();
            for (int Step = 0; Step < productCommentslist.Count(); Step++)
            {
                var item = productCommentslist[Step];
            ...
            for (int Step = 0; Step < pictures.Count(); Step++)
            {
                ...
                //LargePathUrl = LargePathUrl.Replace("//s.trinnk.com", "UserFiles");
            ...
            //{
            //    string largePath = Server.MapPath(item.LargePath.Replace("//s.trinnk.com", "~/UserFiles"));
            ...
            //        }
            //        bool thumbResult = NeoSistem.Trinnk.Core.Web.Helpers.ImageProcessHelper.ImageResize(mainPicture, destinationfile, thumbSizes);
            //    }
            //}
        ...
        public JsonResult FastMember(MembershipModel model, string PhoneNumber, int? productId, string memberEmail1, string content, string subject)
        {
            ...
            if (PhoneNumber != "")
            {
                ...
                NeoSistem.Trinnk.Web.Helpers.SmsHelper sms = new NeoSistem.Trinnk.Web.Helpers.SmsHelper();
        ...
        public string ProductMessageSender(string content, string subject, int? productId, string error, string phoneCode, int senderMainPartyId, string memberEmail, string MailActivationValue)
        {
            ...
            if (MailActivationValue == "0" || MailActivationValue == "")
            {
                ...
                else
                {
                    ...
                    string productUrl = NeoSistem.Trinnk.Core.Web.Helpers.Helpers.ProductUrl(product.ProductId, productName);
