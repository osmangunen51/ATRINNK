...
using NeoSistem.Trinnk.Web.Models.Statistics;
...
namespace NeoSistem.Trinnk.Web.Areas.Account.Controllers
{
    public class StatisticController : BaseAccountController
    {
        ...
        public ActionResult Index()
        {
            ...
            var tmplist=_productStatisticService.GetProductStatisticsByMemberMainPartyId(memberMainPartyId);
            tmplist = tmplist.Where(x => x.RecordDate.Date >= model.startDate && x.RecordDate.Date <= model.endDate).ToList();
            var ProductNoList = tmplist.Select(x => x.ProductId).Distinct().ToList();
            var products = _productService.GetProductByProductsIds(ProductNoList);
            model.StatisticList.Clear();
            foreach (var item in tmplist)
            {
                var product = products.FirstOrDefault(x=>x.ProductId==item.ProductId);
                var ProductStatisticListItem = new ProductStatisticListItem()
                {
                    Product = product,
                    ProductStatistic = item
                };
                model.StatisticList.Add(ProductStatisticListItem);
            }
            return View(model);
        }
        ...
        [HttpPost]
        public ActionResult Index(StatisticModel model)
        {
            int memberMainPartyId = AuthenticationUser.CurrentUser.Membership.MainPartyId;
            var tmplist = _productStatisticService.GetProductStatisticsByMemberMainPartyId(memberMainPartyId);
            tmplist = tmplist.Where(x => x.RecordDate.Date >= model.startDate && x.RecordDate.Date <= model.endDate).ToList();
        ...
        //public void PrepareSatisticLocation(List<global::Trinnk.Entities.Tables.Stores.StoreStatistic> storeStatistics, MTStoreStatisticViewModel model)
        //{
        ...
        //}
        //public MTStatisticModel PrepareStatisticStore(List<global::Trinnk.Entities.Tables.Stores.StoreStatistic> storeStatistics, int storeMainPartyId)
        //{
        ...
        //    }
        //    var productStatistics = new List<global::Trinnk.Entities.Tables.Catalog.ProductStatistic>();
