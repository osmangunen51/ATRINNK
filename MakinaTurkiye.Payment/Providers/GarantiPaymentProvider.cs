...
namespace MakinaTurkiye.Payment.Providers
{
    public class GarantiPaymentProvider : IPaymentProvider
    {
        private readonly HttpClient client;

        public GarantiPaymentProvider()
        {
        ...
        public Task<PaymentGatewayResult> ThreeDGatewayRequest(PaymentGatewayRequest request)
        {
            try
            {
                ...
                string amount = Convert.ToDecimal((request.TotalAmount).ToString("0.##", new CultureInfo("tr-TR"))).ToString("0.00");//virgülden sonraki sıfırlara gerek yok
        ...
        public Task<VerifyGatewayResult> VerifyGateway(VerifyGatewayRequest request, PaymentGatewayRequest gatewayRequest, FormCollection form)
        {
            if (form == null)
            {
