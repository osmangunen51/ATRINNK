...
namespace Trinnk.Utilities.HttpHelpers
{
    public static class UrlBuilder
    {
        private static readonly TrinnkConfig config = EngineContext.Current.Resolve<TrinnkConfig>();
        ...
        private static string GetHost(HostNameType hostNameType, bool ForceGlobalRequest = false)
        {
            if (!ForceGlobalRequest)
            {
                ...
                {
                    return IsRequestLocal ? string.Empty : "http://www.trinnk.com";
                }
            }
            ...
            switch (hostNameType)
            {
                case HostNameType.Product:
                    return Trinnk.Core.AppSettings.UrunUrlBase;
                case HostNameType.Video:
                    return Trinnk.Core.AppSettings.VideoUrlBase;
                case HostNameType.Store:
                    return Trinnk.Core.AppSettings.StoreAllUrl;
                case HostNameType.StoreNews:
                    return Trinnk.Core.AppSettings.HaberUrlBase;
                default:
                    return Trinnk.Core.AppSettings.SiteUrlWithoutLastSlash;
            }
        }
