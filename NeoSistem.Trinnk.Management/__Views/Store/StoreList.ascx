<%@ Control Language="C#" Inherits="NeoSistem.Trinnk.Core.Web.ViewUserControl<FilterModel<StoreModel>>" %>
<% int row = 0; %>
...
<tr id="row<%: item.MainPartyId %>" class="<%: (row % 2 == 0 ? "Row" : "RowAlternate") %>">
    ...
    <td class="Cell">
     ...
     <%var mem = (from c in new TrinnkEntities().MemberStores
    ...
    <td class="CellEnd" align="center" style="width:15%">
        ...
        <%if (!string.IsNullOrEmpty(item.StoreUrlName))
            {%>
        <a title="Detay" href="<%= AppSettings.TrinnkWebUrl + "/"+item.StoreUrlName %>"
            target="_blank">
        ...
        <%}
            ...
            {%>
        <a title="Detay" href="<%= AppSettings.TrinnkWebUrl + "/sirket/"+item.MainPartyId+"/"+Helpers.ToUrl(item.StoreName)+"/sirketprofili" %>"
            target="_blank">
            <img src="/Content/images/rightAllow.png" />
        </a>
        <%} %>
        ...
        <%} %>
                    <%if (NeoSistem.Trinnk.Management.Models.Authentication.CurrentUserModel.CurrentManagement.UserId == 1) {
                    %>
